...
- **API 키 자동 저장**: 한 번 입력하면 다음 실행 시 자동 로드
...
### 3. Excel 뷰어/에디터 ✨ 신규
- **프로그램 내에서 Excel 파일 직접 확인**
- **셀 편집**: 더블클릭으로 값 수정
- **행 삭제**: 여러 행 선택 후 일괄 삭제
- **변경사항 저장**: 원본 파일에 저장 또는 다른 이름으로 저장
- **실시간 통계**: 총 행 수 표시
- CSV 및 Excel 파일 지원
...
1. "찾아보기" 버튼으로 Excel/CSV 파일 선택
2. "열기" 버튼 클릭하여 파일 로드
3. **데이터 보기**: 테이블 형태로 전체 데이터 확인
4. **셀 편집**: 편집할 셀을 더블클릭 → 값 수정 → 확인
5. **행 삭제**:
   - 삭제할 행 클릭 (Ctrl/Cmd+클릭으로 여러 행 선택)
   - "선택 행 삭제" 버튼 클릭
6. **저장**:
   - "변경사항 저장": 원본 파일에 덮어쓰기
   - "다른 이름으로 저장": 새 파일로 저장
...
### API 키 자동 저장 ✨ 신규
- **한 번만 입력**: API 키를 입력하고 "저장" 버튼 클릭
- **자동 로드**: 프로그램 재실행 시 자동으로 API 키 로드
- **안전한 저장**: 홈 디렉토리의 숨김 폴더에 안전하게 저장 (~/.paper_search_filter/config.json)
- **편리한 사용**: 매번 API 키를 입력할 필요 없음

### Excel 뷰어/에디터 ✨ 신규
- **프로그램 내 확인**: Excel 파일을 별도 프로그램 없이 바로 확인
- **직관적인 인터페이스**: 테이블 형태로 모든 데이터 표시
- **셀 편집**: 더블클릭으로 간편하게 값 수정
- **행 삭제**: 여러 행을 한 번에 선택하여 삭제
- **즉시 저장**: 변경사항을 바로 저장하거나 새 파일로 저장
- **실시간 통계**: 총 행 수 등 통계 정보 표시
